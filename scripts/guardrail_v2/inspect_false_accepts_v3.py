...
PATHS = [
    ("held-out", "data/guardrail/negatives/dtd/0350.jpg", 0.935, 0.001),
    ("held-out", "data/guardrail/negatives/dtd/0133.jpg", 0.807, 0.011),
    ("fresh", "data/guardrail/fresh_negatives/dtd/0059.jpg", 0.815, 0.000),
]
...
if __name__ == "__main__":
    ...
    for tag, p, p2, p1 in PATHS:
        ...
        print(f"{tag:8s} {p:52s} v3 P(smear) {p2:.3f} | detector cells {cells:3d}, top cell score {top:.3f} -> pipeline answer: {ans}")
        t = cv2.resize(img, (240, 240)); cv2.putText(t, f"v3 {p2:.2f} cells {cells}", (4, 18), 0, 0.55, (0, 255, 255), 2)
        tiles.append(t)
    cv2.imencode(".png", np.hstack(tiles))[1].tofile(os.path.join("model_output", "guardrail", "phase14b_false_accepts_v3.png"))
